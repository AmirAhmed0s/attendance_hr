...
from hrms.doctype.holiday_list.holiday_list import HolidayList
from hrms.doctype.leave_application.leave_application import get_leave_balance_on, is_lwp
import frappe
...
# from hrms.doctype.shift_assignment.shift_assignment import get_shift_details
from hrms.doctype.employee.employee import get_holiday_list_for_employee, is_holiday
...
class AttendanceCalculation(Document):
    ...
    def validate_dates(self):
        ...
        if not (self.payroll_start_date <= self.start_date <= self.payroll_end_date):
            frappe.thrmsow(_("Start Date is Not in Payroll Period Date Range"))

        if not (self.payroll_start_date <= self.end_date <= self.payroll_end_date):
            frappe.thrmsow(_("End Date is Not in Payroll Period Date Range"))
    ...
    def get_attendance(self, employee, day):
        ...
        if exist:
            ...
            if doc.attendance_calculation:
                ...
                frappe.thrmsow(_(f"Please delete Attendace {link} First"))
        else:
            return frappe.new_doc("Attendance")
    ...
    def submit_additional_salary(self, employee, amount, salary_component, salary_component_type, remark, mark):
        ...
        try:
            ...
            doc.naming_series = "hrms-ADS-.YY.-.MM.-"
            doc.employee = employee.name
